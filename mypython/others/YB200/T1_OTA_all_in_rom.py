...
def Result_all():
    ...
    if resultsrom.find("stm32") == -1:
        log.info("can't find stm32 log")


            
    if resultsrom.find("fpga") != -1:
        if resultsrom.find("persist.sys.fpgaloaded=1, error: don't need update") != -1:
            log.info("persist.sys.fpgaloaded=1, no need update fpga")
            
        elif resultsrom.find("set persist.sys.upfpga status: 0") != -1 or resultsrom.find("result_fpga=failed") != -1:
...
if __name__ == '__main__':
    ...
    for p in range(1,testtime):
        ...
        try:
            ...
            log.info("----- unknown error happened,restart test again -----\n")
