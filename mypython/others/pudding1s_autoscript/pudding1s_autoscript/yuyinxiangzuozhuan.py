...
def yuyinxiangzuozhuan():
	...
	if  "root" not in output:
		...
		os.system(command)	
	rmlog="adb "+ sn +"shell \"rm -fr /sdcard/yuyinxiangzuozhuan.log \""
	os.system(rmlog)
	cppcm="adb "+ sn +"shell \"busybox cp /sdcard/nihao.pcm /sdcard/test_wakeup.pcm \""
	os.system(cppcm)
	pidstart = 0
	while(pidstart<1):
		pidlogcat="adb "+ sn +"shell \"busybox nohup logcat -v time  -f /sdcard/yuyinxiangzuozhuan.log & \""
	...
	while(wakeup<1):
		...
		os.system(amcommand)
		time.sleep(3)
		checkloghx="adb "+ sn +"shell \"cat /sdcard/yuyinxiangzuozhuan.log |grep -E '唤醒|你好'\""
	...
	time.sleep(10)	    		
	cpclock="adb "+ sn +"shell \"busybox cp /sdcard/xzz.pcm /sdcard/test_wakeup.pcm \""
	os.system(cpclock)
	os.system(amcommand)
	time.sleep(10)
		
	checklogclock="adb "+ sn +"shell \"cat /sdcard/yuyinxiangzuozhuan.log |grep paramJasonStr\""
	output = os.popen(checklogclock)
	time.sleep(3)
	res= output.read()
	print "turn left logcat",res
	
	if '向左转' in res:
		print 'set turn left successful';
	else:
		print 'set turn left failed'
		return False
	...
	while(sleep<1):
		...
		os.system(amcommand)
		time.sleep(3)
		checklogsleep="adb "+ sn + "shell \"cat /sdcard/yuyinxiangzuozhuan.log |grep paramJasonStr\""
